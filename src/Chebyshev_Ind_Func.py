...
for i in range(len(approx_x)):
    approx_y[i] = unit(approx_x[i])
# approx_y = unit(approx_x)
# approx_y = np.sin(approx_x)
approx_s = approx_x*0# np.cos(approx_x)

# output = optimize.fmin(approx_objective, x0=[1]*degree,
#                        args=(approx_x, approx_y, approx_s), maxfun=10000)
V = vander(approx_x, 20, np.min(approx_x), np.max(approx_x))
V1 = vander1(approx_x, 8, np.min(approx_x), np.max(approx_x))
...
for j in range(len(xx)):
    approx_y1[j] = V_hat(xx[j], b, np.min(approx_x), np.max(approx_x))

approx_s1 = [0]*len(xx)
for j in range(len(xx)):
    approx_s1[j] = V_hat(xx[j], b0[0], np.min(approx_x), np.max(approx_x))+.001
...
plt.legend(['(x>1/2)?1:0', 'Lagrange w/ degree=11', 'Hermite w/ degree=20'], loc='best')
